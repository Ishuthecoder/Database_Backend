...
from phi.tools.googlesearch import GoogleSearch as PhiGoogleSearch
...
class CustomGoogleSearch:
    """
    Custom GoogleSearch implementation for specific website crawling.
    """
    def __init__(
        self,
...
# Initialize custom Google search
custom_google_search = CustomGoogleSearch()

@app.route('/api/custom-search', methods=['POST'])
def custom_search():
    """
    Endpoint for custom Google search with site-specific filtering.
    """
    try:
        data = request.json
        query = data.get('query', '')
        ...
        # Perform the search
        results = custom_google_search.search(query, max_results=max_results, language=language)
